...
1. **错位重复**：`V1 == a & V2 == b` 和 `V1 == b & V2 == a` 也视为重复。
...
1. 使用 `sort()` 对 `total` 列中的每一个向量进行排序。由于 `total` 列是一个列表，对其操作需使用 `map()` ：
...
在列数更多的情况下，利用 `c_across()` 对不同列进行拼接转为向量，可以减少输入列名的麻烦。

# 思考

## 对数据的认识

**tibble** 支持数据的嵌套（如 Figure 1）和 list 列（上述的 `total` 列）。这二者都是我之前比较畏惧的内容，因为对它们的处理涉及到 `map()` 系列函数（**purrr**），对我来说是个难点。~~现在稍微好一点。~~
